  ...
  <b-card class="m-2">
    ...
    >
      ...
      <template #item-actions="items">
        <b-button
          @click="RemoveDocument(items.archivoArmeriaId)"
          class="m-1"
...
export default {
  ...
  setup (props) {
    ...
    const equipoComputoId = ref(props.ArmaId)
    ...
    getDocumentsArmary(props.ArmaId, data => {
    ...
    const submitFiles = () => {
      ...
      createDocumentsArmary(props.ArmaId, formData, data => {
    ...
    const onDownloadFile = (ArchivoArmeria) => {
      isloading.value = true
      disableButtonDownload.value = true
      axiosPrivate({
        ...
        .then(response => {
          ...
          link.setAttribute(
            'download',
            `${ArchivoArmeria.nombre}${ArchivoArmeria.tipoArchivo}`
          )
          document.body.appendChild(link)
          link.click()
        })
    ...
    const RemoveDocument = archivoId => {
      swal
        ...
        .then(result => {
          if (result.isConfirmed) {
            deleteDocumentArmary(props.ArmaId, archivoId, data => {
              swal
                .fire({

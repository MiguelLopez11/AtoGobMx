  ...
  <b-card class="m-2">
    ...
    >
      ...
      <template #item-actions="items">
        <b-button
          @click="RemoveDocument(items.archivoEquipoId)"
          class="m-1"
...
export default {
  ...
  setup (props) {
    ...
    const equipoComputoId = ref(props.EquipoId)
    ...
    getDocumentsEquipmentComputer(props.EquipoId, data => {
    ...
    const submitFiles = () => {
      ...
      createDocumentsEquipmentComputer(props.EquipoId, formData, data => {
    ...
    const onDownloadFiles = () => {
      ...
      axiosPrivate({
        url: `/Archivos/DocumentosEquiposComputo/${props.EquipoId}/Zip`,
        method: 'GET',
    ...
    const onDownloadFile = (ArchivoEquipo) => {
      isloading.value = true
      disableButtonDownload.value = true
      axiosPrivate({
        url: `/Archivos/DocumentosEquiposComputo/Descargar/${props.EquipoId}/${ArchivoEquipo.archivoEquipoId}`,
        method: 'GET',
        responseType: 'blob' // important
      })
        .then(response => {
          ...
          link.setAttribute(
            'download',
            `${ArchivoEquipo.nombre}${ArchivoEquipo.tipoArchivo}`
          )
          document.body.appendChild(link)
          link.click()
        })
    ...
    const RemoveDocument = archivoId => {
      swal
        ...
        .then(result => {
          if (result.isConfirmed) {
            deleteDocumentEquipmentComputer(props.EquipoId, archivoId, data => {
              swal
                .fire({

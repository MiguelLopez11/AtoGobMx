...
namespace AtoGobMx.Controllers
{
    ...
    public class ControlDeValesController : ControllerBase
    {
        ...
        public async Task<ActionResult<PROV_ControlVales>> PostControlDeVales(PROV_ControlVales controlDeVales)
        {
            ...
            controlDeVales.EstatusValeId = 1;
        ...
        [HttpPut("Autoriza/{ControlValeId}")]
        public async Task<ActionResult> AutorizaControlDeVales(int ControlValeId)
        {

            var controlvale = await _context.ControlDeVales.FirstOrDefaultAsync(f => f.ControlValeId == ControlValeId);
            if (controlvale == null)
            {
                return BadRequest("Control de vales no existe");
            }
            controlvale.Autorizado = true;
            controlvale.EstatusValeId = 2;

            _context.ControlDeVales.Update(controlvale);
            await _context.SaveChangesAsync();
            return Ok("Control de vales actualizado correctamente");
        }
        [HttpPut("Entrega/{ControlValeId}")]
        public async Task<ActionResult> EntregarControlDeVales(int ControlValeId)
        {
            ...
            if (!controlvale.Autorizado)
            {
                return BadRequest("Control de vales no autorizado");
            }
            controlvale.EstatusValeId = 3;
            _context.ControlDeVales.Update(controlvale);
            await _context.SaveChangesAsync();
            return Ok("Control de vales actualizado correctamente");
        }
        [HttpPut("Recibir/{ControlValeId}")]
        public async Task<ActionResult> RecibirControlDeVales(int ControlValeId)
        {
